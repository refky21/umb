...
License: GPLv2
*/
define('PLUGIN_BASENAME_URI', THEME_PLUGINS_URI.'/magic-metabox/');
define('PLUGIN_BASENAME_DIR', THEME_PLUGINS_DIR.'/magic-metabox/');
...
class MagicMetaBox {
    ...
    public function __construct($id, $opts) {
       	require_once PLUGIN_BASENAME_DIR . 'shortcode/shortcode.php';
        if (!is_admin())
            return;
        $this->meta_box = $opts;
        $this->id = $id;
        ...
        require_once PLUGIN_BASENAME_DIR . 'includes/metabox-ajax.php';
    }

    function gsmb_add_metabox_style($page) {
        if (is_admin()) {

            if( in_array( $page, array( 'edit-tags.php', 'term.php' ) ) ) return;

            wp_enqueue_style('wp-color-picker');
			wp_register_style('fonts', PLUGIN_BASENAME_URI .'css/fonts.css');
            wp_enqueue_style('fonts');
			
            wp_register_style('jqueryUICSS', PLUGIN_BASENAME_URI .'css/jquery-ui.min.css');
            if (isset($_GET['page'])) {
                if (strpos($_GET['page'], 'wpcf7')){
                    wp_enqueue_style('jqueryUICSS');
                }
            }
            
            wp_register_style('select2CSS', PLUGIN_BASENAME_URI .'css/select2.min.css');
            wp_enqueue_style('select2CSS');
            
            wp_register_style('jquery-ui-timepicker-addon-css', PLUGIN_BASENAME_URI .'css/jquery-ui-timepicker-addon.min.css');
            wp_enqueue_style('jquery-ui-timepicker-addon-css');
            
            /* style Theme Option */
            wp_register_style('metaBoxGeneratorCSS', PLUGIN_BASENAME_URI .'css/metabox.css');
    ...
    function gsmb_add_metabox_script() {
        if (is_admin()) {
            ...
            wp_register_script('mapJS', PLUGIN_BASENAME_URI .'js/meta-box-map.js', array('jquery'));

            wp_register_script('select2JS', PLUGIN_BASENAME_URI .'js/select2.min.js', array('jquery'));
            wp_enqueue_script('select2JS');

            wp_register_script('jquery-ui-timepicker-addon-js', PLUGIN_BASENAME_URI .'js/jquery-ui-timepicker-addon.min.js', array('jquery-ui-datepicker', 'jquery-ui-slider', 'jquery'));
            wp_enqueue_script('jquery-ui-timepicker-addon-js');

            wp_register_script('metaboxGallery', PLUGIN_BASENAME_URI .'js/metabox-gallery.js', array('jquery'));
            ...
            wp_register_script('metaboxImage', PLUGIN_BASENAME_URI .'js/meta-box-image.js', array('jquery'));
            ...
            wp_register_script('metaboxFile', PLUGIN_BASENAME_URI .'js/meta-box-file.js', array('jquery'));
            ...
            wp_register_script('metabox_js', PLUGIN_BASENAME_URI .'js/metabox.js', array('wp-color-picker', 'jquery-ui-datepicker', 'jquery-ui-spinner', 'jquery-ui-slider'));
			
			wp_register_script('show-hide-metabox', PLUGIN_BASENAME_URI .'js/show-hide-metabox.js', array('jquery'));
        }
    }
