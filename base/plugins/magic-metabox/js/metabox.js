...
$(document).ready(function() {
	...
	var textarea_id_wp_editor = new Array();
	...
	$('.textareaTinyMCE').each(function(index){
		...
		tinymce.init({
	        ...
	        toolbar3: "table | emoticons ltr rtl | spellchecker | visualchars visualblocks nonbreaking template pagebreak restoredraft | code",
	...
	$('.wpeditorTinyMCE').each(function(index) {
		var textarea_id = $(this).attr('id');
		textarea_id_wp_editor[index] = textarea_id;
		wp.editor.initialize(
			...
			{ 
				tinymce: { 
					...
					toolbar1: 'bullist numlist | link unlink' 
				}, 
				quicktags: true 
			}
		);
	...
	$(".addNewMeta").unbind('click').click(function(e) {
	    ...
	    $(this).closest('.containerAddNew').find('.contentRepeater .repeaterWrapper').last().find('*').each(function(){
	        ...
	        if($(this).attr('data-wp-editor-id') != '' && $(this).attr('data-wp-editor-id') != undefined ){
	           attrEditor += $(this).attr('data-wp-editor-id').replace(/-[0-9]+-/g, '-' + count[number_metabox][number_repeat] +'-');
	           $(this).attr('data-wp-editor-id', attrEditor);
	        }
	    ...
	    // Add New Upload File
	    $(this).closest('.containerAddNew').find('.repeaterWrapper').last().find('.imageUploadMeta').each(function(){
	    	$(this).find('.metaboxImageTitle a').attr('href', '');
	    	$(this).find('.metaboxImageTitle p').html('');
	    });
	    ...
	    // Add New WP Editor
	    $(this).closest('.containerAddNew').find('.repeaterWrapper').last().find('.wp-editor-wrap').each(function(){
	    	var textarea_id = $(this).parent().data('wp-editor-id');
	    	var textarea_name = $(this).parent().data('wp-editor-name');
	    	$(this).closest('.wpeditorContentMetaDesc').addClass('here');
	    	$(this).parent().find('textarea').remove();
	    	$(this).remove();
	    	textarea_name = textarea_name.replace(/[[][0-9]+]/g, '[' + (count[number_metabox][number_repeat] - 1) +']')
	    	textarea_id_wp_editor[textarea_id_wp_editor.length] = textarea_id;

	    	console.log(textarea_name);
    		$('.wpeditorContentMetaDesc.here').append('<textarea id="'+ textarea_id +'" name="'+ textarea_name +'" style="min-height: 200px;"></textarea>');
	    	$('.wpeditorContentMetaDesc.here').removeClass('here');
	    	wp.editor.initialize(
				textarea_id,
				{ 
					tinymce: { 
