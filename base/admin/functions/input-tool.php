...
class input_tool {
	...
	function generate_meta_option() {
		...
		foreach( $this->options as $option ){
			$toggle_group = ( isset($option['toggle_group']) ) ? ' input-list-toggle ' . $option['toggle_group'] : '';
			if( $option['type'] == 'separator' ) {
				echo '<tr class="separator"><td colspan="2"><strong>';
				if( isset( $option['title'] ) ) echo $option['title'];
				echo '</strong></td></tr>';
			} else {
				echo '<tr class="input-list' . $toggle_group . '">';
				...
				echo '</td>';
				echo '<td class="input-field">';
					$this->{$option['type']}($option);				echo '</td>';
				echo '</tr>';
			}
		}
		echo '</table>';
	...
	function number( $option ) {
		...
		echo '<input type="number" class="input-number" id="' . $this->id( $option ) . '" name="' . $this->name( $option ) . '" value="' . stripslashes(htmlspecialchars($value)) . '"' . $min . ' ' . $max . '>';
	}
	...
	function editor( $option ) {
		...
		//echo '<input type="text" class="input-text" id="' . $this->id( $option ) . '" name="' . $this->name( $option ) . '" value="' . stripslashes(htmlspecialchars($value)) . '">';
		$textarea_name = $this->name( $option );
		...
		if ( isset( $option['settings']) ) {
			$editor_settings = $option['settings'];
		}
		$editor_settings = array_merge( $default_editor_settings, $editor_settings );
		wp_editor( $value, $this->id( $option ), $editor_settings );
	}
	...
	function checkbox( $option ) {
		...
		$column = ( isset($option['layout'])) ? 'input-field-row '.$option['layout'] : 'input-field-row';
	...
	function select( $option ) {
		...
		if( isset ( $option['source']['taxonomy'] ) ) {
			$args = array( 'type' => 'post', 'taxonomy' => $option['source']['taxonomy'], 'orderby' => 'term_group', 'hide_empty' => false );
			$taxanomies = get_categories( $args );
			foreach( $taxanomies as $taxanomy ) {
				$source[ $taxanomy->term_id ] = $taxanomy->name;
			}
		}

		// Source : Page Template
		if( isset ( $option['source']['page_template'] ) ) {
			query_posts(array(
		    	'post_type'  	=> 'page',  /* overrides default 'post' */
			    'meta_key'   	=> '_wp_page_template',
			    'meta_value' 	=> $option['source']['page_template'],
			    'order'			=> 'ASC',
			    'orderby'		=> 'date'
			));
			...
			if (have_posts()) : 
				while (have_posts()) : the_post();
					$source[ get_the_id() ] = get_the_title(). ' (' . get_the_id() . ')';
				endwhile;
			endif; 
		...
		// Source : Custom Query
		if(isset($option['source']['custom_query'])){
			query_posts($option['source']['custom_query']);
		...
		// Placeholder
		$placeholder = ( isset( $option['placeholder'] ) ) ? ' placeholder="'.$option['placeholder'].'" data-placeholder="'.$option['placeholder'].'"' : '';

		echo '<select '.$multiple.' class="input-select'.$class.'" ' . $name . '" id="' . $this->id( $option ) . '" ' . $placeholder . '>';
	...
	function date( $option ) {
		...
		echo '<input type="text" class="input-date" id="' . $this->id( $option ) . '" name="' . $this->name( $option ) . '" value="' . $value . '" >';
	...
	function yes_no( $option ) {
		...
		// Sent value even the checkbox is unchecked http://stackoverflow.com/questions/476426/submit-an-html-form-with-empty-checkboxes
		echo '<input type="hidden" name="' . $this->name( $option ) . '" value="no" />';
		echo '<input type="checkbox" class="input-on-off input-yes-no"  name="' . $this->name( $option ) . '" id="' . $this->id( $option ) . '" value="yes" ' . $checked . $toggle . ' />';
	}
